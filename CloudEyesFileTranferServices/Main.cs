...
using System.IO;
using CloudEyesFileTranferServices;
...
public partial class Main : Form
{
    ...
    private void btnStartServer_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            txtServerPort.Text = "8080";//端口服务器端口号，IP地址在listener.cs里边设置
            listener.Start(int.Parse(txtServerPort.Text.Trim()));
    ...
    private void autoRunServer() //自动运行server服务
    {
        if (serverRunning)
            return;
        serverRunning = true;
        ...
        {
            //Try to listen on the desired port
            txtServerPort.Text = "8000";//端口服务器端口号
            listener = new Listener();
            listener.Start(8000);
            //Set the connection status to waiting
            setConnectionStatus("Waiting...");
        ...
        {
            MessageBox.Show("Unable to listen on port " + txtServerPort.Text, "", MessageBoxButtons.OK, MessageBoxIcon.Error);
            MessageBox.Show("Class B Error：" + e);
        }
    }
    ...
    private void txtCntPort_Click(object sender, EventArgs e)
    {
