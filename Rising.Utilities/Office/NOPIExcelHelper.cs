...
{
    /// <summary>
    /// NOPIExcelHelper,采用NOPI2.1.3.1实现
    /// </summary>
    public class NOPIExcelHelper
    {
        #region 成员变量
        private string templetFile = null;      // Excel模板文件路径
        private string outputFile = null;       // 输出Excel文件路径
        ...
        {
            get { return workBook.NumberOfSheets; }
        }
        ...
        public NOPIExcelHelper(string templetFilePath, string outputFilePath)
        {
            ...
            using (FileStream file = new FileStream(templetFilePath, FileMode.Open, FileAccess.Read))
            {
        ...
        public void ChangeCurrentWorkSheet(int sheetIndex)
        {
            ...
            this.workSheet = workBook.GetSheetAt(sheetIndex);
        }
        ...
        public string GetCell(int rowIndex, int columnIndex)
        {
            ...
            {
                throw new Exception("读取单元格[" + rowIndex + "," + columnIndex + "]数据出错！");
            }
            ...
            for (int i = 0; i < this.WorkSheetCount; i++)
            {
                ...
                foreach (DictionaryEntry dic in ht)
                {
                    try
                    {
                        ...
                        //TODO：不支持Object类型，仅支持可见字符串，后期考虑优化
                        cell.SetCellValue(dic.Value.ToString());
                    }
                    catch
                    {
        ...
        public void InsertRows(int sheetIndex, int rowIndex, int count)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    workSheet.CreateRow(rowIndex + i);
                }
            }
            catch (Exception e)
            {
        ...
        public void OutputExcelFile()
        {
            ...
            try
            {
                using (FileStream file = File.Create(this.outputFile))
                {
                    workBook.Write(file);
                }
            }
